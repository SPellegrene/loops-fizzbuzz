...
function isValid(ccnum) {
  ...
  // First strip the spaces out, the total length may be !16
  var ccNoSpaces = ccnum.replace(/\s/g, '');

  // Ensure there are exactly 16 non-whitespace characters
  if (ccNoSpaces.length != 16) {
    return false;  // If not, exit immediately
  }

  // We need a string to hold all our digits!
  var digitString = '';

  // Loop through the CC# from right to left
  for (var i = ccNoSpaces.length-1; i >= 0; i--) {
    var digit = ccNoSpaces[i]; // get the current digit

    // Determine if we need to double the current digit or not
    if(i % 2) { // Evens, double it
      digitString += '' + (digit*2) + '';  // Use string concat
    } else { // Odds,
      digitString += '' + digit + ''; // Use string concat
    }
  }

  // Instead of a big string, we want an array, so split it
  var arrayOfDigits = digitString.split('');

  // Here we're just taking an array of strings that looks like:
  //    ['6','4','7','4','2' ...etc
  // and summing them
  var total = arrayOfDigits.reduce(function(a,b) {
    // Sum the numbers, converting them to integers first
    // remember that right now they are strings
    return parseInt(a) + parseInt(b);
  }, 0);

  // check if it evenly divisible by 10
  // if it is, it is valid, if not, invalid.
