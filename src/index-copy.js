...
// format in a way we can use
var ccnum2 = ccnum.replace(/ /g, "").split("").map(Number);

// Double every number
  for (var i = ccnum.length; i >= 0; i-2) {
  ccnum2[i]=ccnum2[i] * 2;
  return ccnum2;
}
//sum the doubled and untouched digits
  var added =(ccnum + ccnum2);

// if the sum of both doubled and untouched is a multiple of 10 make it true
  if (added % 10 === 0){
    return true;
  } else{
    return false;
